<?php
/**
 * Registers the md/sync-product-from-amazon block.
 *
...
/**
 *  Class for the md/sync-product-from-amazon block.
 */
class Sync_Product_From_Amazon extends Block_Base {
	 ...
	 * @since    1.0.0
	 * @access   private
	 * @var      string    $md_app_field_options    Settings of this plugin.
	 */
	private $md_app_field_options;
	...
	protected function setup_hooks() {
		$this->md_app_field_options = get_option( 'md_app_field_options' ) ?? array();
	}
			...
			<div <?php echo wp_kses_post( $wrapper_attributes ); ?>>
				<div class="sync-product-from-amazon">
					<div class="sync-product-from-amazon__left_wrapper">
						<?php
						if ( ! empty( $this->md_app_field_options['show_product_image'] ) ) {
							?>
						...
						<div class="sync-product-from-amazon__details_wrap">
							<div class="sync-product-from-amazon__price_detail">
								<?php
								if ( ! empty( $this->md_app_field_options['show_product_price'] ) ) {
									?>
							...
							<?php
							if ( ! empty( $this->md_app_field_options['show_cta_link'] ) ) {
								?>
					...
					<div class="sync-product-from-amazon__content">
						<?php
						if ( ! empty( $this->md_app_field_options['show_product_title'] ) ) {
							?>
						...
						?>
						<?php
						if ( ! empty( $this->md_app_field_options['show_product_features'] ) ) {
							?>
